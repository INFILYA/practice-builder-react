          ...
          {plans.map(plan => {
            ...
            return (
              <div key={plan.key}
                ...
                <p className="font-condensed text-lg font-bold">{plan.title || 'Untitled'}</p>
                ...
                <p className="text-xs text-gray-600 mt-1">by {plan.authorName}</p>
                <div className="flex flex-wrap gap-1 mt-2">
                  {mods.map(mod => (
                    <span key={mod} className={`text-xs font-bold px-1.5 py-0.5 rounded font-condensed ${(MODULE_COLORS[mod] ?? MODULE_COLORS.M1).badge}`}>
                      {mod}
                    </span>
                  ))}
                </div>
                <div className="flex gap-2 mt-3">
                  <button onClick={() => onLoad(plan.key, plan)}
                    className="px-3 py-1 rounded-md text-xs border border-white/10 text-gray-300 hover:text-white hover:bg-bg3 transition-all">
                    Open
                  </button>
                  {isOwn && (
                    <button onClick={() => handleDelete(plan.key)}
