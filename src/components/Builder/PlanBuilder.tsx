...
import { SessionMetaBar } from "./SessionMeta";
...
import type { PlanBlock, SessionMeta } from "../../types";
...
export function PlanBuilder({
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <SeasonTemplate totalDurationMins={maxMins} activePhase={activePhase} />
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 scrollbar-thin">
        {blocks.length === 0 ? (
          ...
          <>
            <button
              onClick={onAddWarmup}
              className="w-full mb-1.5 py-1.5 rounded-md border border-dashed border-white/10 text-xs text-gray-600 hover:text-gray-400 hover:bg-bg3 transition-all"
            >
              + Add warm-up
            </button>
            {blocks.map((block) => (
            ...
            <button
              ...
              className="w-full mt-1 py-1.5 rounded-md border border-dashed border-white/10 text-xs text-gray-600 hover:text-gray-400 hover:bg-bg3 transition-all"
            >
              + Add cool-down
            </button>
          </>
