import { useState } from 'react'
import type { AuthError } from 'firebase/auth'

interface Props { onSignIn: () => Promise<unknown> }

export function AuthScreen({ onSignIn }: Props) {
  const [error, setError] = useState('')
  const handleSignIn = async () => {
    try { setError(''); await onSignIn() }
    catch (e) { setError((e as AuthError).message ?? 'Sign in failed') }
  }
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-8"
         style={{background:'radial-gradient(ellipse at 30% 40%,rgba(46,134,222,0.08) 0%,transparent 60%),radial-gradient(ellipse at 70% 70%,rgba(240,180,41,0.06) 0%,transparent 55%),#0d0f14'}}>
      <div className="text-center">
        <h1 className="font-condensed text-6xl font-black tracking-tight leading-none">
          Practice<span className="text-accent">Builder</span>
        </h1>
        <p className="text-gray-500 text-sm mt-2 uppercase tracking-widest">Practice planning & sessions</p>
      </div>
      <div className="bg-bg2 border border-white/10 rounded-xl p-8 w-80 flex flex-col gap-4">
        <h2 className="font-condensed text-xl font-semibold">Sign in to continue</h2>
        <button onClick={handleSignIn}
          className="flex items-center justify-center gap-3 w-full py-3 px-5 rounded-lg border border-white/10 bg-bg3 text-sm font-medium transition-all hover:border-accent hover:text-accent">
          <svg viewBox="0 0 24 24" className="w-5 h-5 flex-shrink-0">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
          </svg>
          Continue with Google
        </button>
        {error && <p className="text-red-400 text-xs">{error}</p>}
      </div>
    </div>
  )
}
